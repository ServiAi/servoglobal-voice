...
El despliegue actual usa LiveKit Server self-hosted. LiveKit Cloud no es requerido; cualquier endpoint compatible se configura mediante `LIVEKIT_URL`.
...
`AgentCompilerService` resuelve el modelo lógico del Agent Builder (`ultravox`) mediante el Provider/Model Registry y entrega al runtime el ID ejecutable (`fixie-ai/ultravox`) sin mutar la versión publicada. `UltravoxLiveKitRuntime` usa el plugin oficial y consume ese ID sin volver a resolverlo. Mapea prompt publicado, voz, idioma base (`es-CO` a `es`), temperatura 0–1 y primer hablante. El turn detection permanece server-side porque el plugin no permite desactivarlo. La interrupción `conservative` desactiva interrupciones; los otros modos usan el soporte disponible de LiveKit. El saludo se incorpora una sola vez al prompt automático para evitar doble reproducción.
