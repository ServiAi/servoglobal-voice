...
function getTitle(pathname: string) {
  if (pathname.includes('/crm/settings/integrations')) return 'Integraciones';
  if (pathname.includes('/crm/dashboard')) return 'Rendimiento';
  if (pathname.includes('/crm/leads/')) return 'Detalle del lead';
  if (pathname.endsWith('/crm/leads')) return 'Leads';
  if (pathname.includes('/crm/tasks')) return 'Tareas';
  return 'Pipeline';
}
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border bg-background/92 px-4 backdrop-blur-md sm:px-6 lg:ml-64">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold sm:text-base">{getTitle(pathname)}</p>
      ...
      >
        ...
        <span className="hidden sm:inline">Métricas de voz</span>
        <span className="sr-only sm:hidden">Métricas de voz</span>
      </Link>
      <ThemeToggle />
