...
export function GoogleCalendarConnectionList({ connections, onDisconnect }: Props) {
  ...
  if (connections.length === 0) {
    return <p className="text-sm text-muted-foreground">Sin conexiones Google Calendar.</p>;
  }
  return (
    <div className="grid gap-2">
      {connections.map((connection) => (
        <div key={connection.id} className="flex items-start justify-between gap-3 rounded-md border border-border p-3 text-sm">
          <div>
            <div className="font-medium text-foreground">{connection.google_account_email ?? 'Cuenta Google'}</div>
            <div className="text-muted-foreground">{connection.calendar_summary ?? connection.calendar_id}</div>
            <div className="mt-1 text-xs text-muted-foreground">{connection.status}</div>
          </div>
