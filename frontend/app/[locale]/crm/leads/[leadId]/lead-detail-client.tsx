...
export function LeadDetailClient({
  ...
  const handleSaveLead = async (payload: LeadUpdateRequest) => {
    ...
    if (res.ok) {
      triggerRefresh('Lead actualizado con éxito.');
    } else {
  ...
  const handleAddNote = async (noteText: string) => {
    ...
    if (res.ok) {
      triggerRefresh('Nota interna agregada con éxito.');
    } else {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Main Details and Edit panel */}
      <CrmLeadDetailPanel
        lead={lead}
        onSave={handleSaveLead}
      />

      {/* Bottom split: Activities Timeline vs Notes & Tasks */}
