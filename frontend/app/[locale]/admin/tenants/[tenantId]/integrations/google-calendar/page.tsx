...
export default async function AdminTenantGoogleCalendarPage({ params }: Props) {
  ...
  return <IntegrationDetailShell locale={locale} integrationsLabel={t('title')} integrationsPath={integrationsPath} name={t('providers.google_calendar.name')} description={t('providers.google_calendar.description')}><GoogleCalendarIntegrationCard connections={result.ok ? result.data : []} /></IntegrationDetailShell>;
}
