...
export default async function AdminTenantWhatsAppOverviewPage({ params }: Props) {
  ...
  const base = `/${locale}${integrationsPath}/whatsapp`;
  const actions = [
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-semibold text-foreground">{t('overview.title')}</h2><p className="mt-1 text-sm text-muted-foreground">{t('overview.description')}</p></div>
      ...
      <section aria-labelledby="admin-whatsapp-quick-actions" className="space-y-3">
        <h2 id="admin-whatsapp-quick-actions" className="text-base font-semibold text-foreground">{t('quickActions.title')}</h2>
        <div className="grid gap-3 sm:grid-cols-3">{actions.map(({ href, icon: Icon, label }) => <Link key={href} href={href} className="group flex min-h-20 items-center gap-3 rounded-xl border border-border bg-card p-4 text-sm font-semibold text-foreground outline-none transition hover:border-primary/30 hover:bg-muted/30 focus-visible:ring-2 focus-visible:ring-ring"><Icon className="size-5 text-primary" aria-hidden="true" /><span className="flex-1">{label}</span><ArrowRight className="size-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></Link>)}</div>
      </section>
    </div>
  );
}
