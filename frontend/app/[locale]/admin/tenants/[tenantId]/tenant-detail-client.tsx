...
function StatusBadge({ status }: { status: string }) {
  if (status === 'active') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400">
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-zinc-500/10 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-500/20 dark:text-zinc-400">
...
function ErrorState({ message }: { message: string }) {
  return (
    <div className="rounded-xl border border-red-300 bg-red-50 p-6 text-center dark:border-red-700 dark:bg-red-950/30">
      <AlertCircle className="mx-auto mb-2 h-8 w-8 text-red-500 dark:text-red-400" />
...
function CopyableField({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-sm text-zinc-700 dark:text-zinc-200">{value}</span>
      <button
        type="button"
        onClick={handleCopy}
        className="rounded p-1 text-zinc-400 transition hover:text-zinc-700 dark:text-zinc-500 dark:hover:text-zinc-200"
        title={`Copiar ${label}`}
      >
        {copied ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
      </button>
    </div>
  );
}
...
export function TenantDetailClient({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <Link
            ...
            className="mb-3 inline-flex items-center gap-1.5 text-sm text-zinc-500 transition hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200"
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-red-300 px-3 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-50 dark:border-red-700 dark:text-red-400 dark:hover:bg-red-950/30"
            title="Borrar tenant"
          >
      ...
      {/* Integration Info */}
      <section className="mb-8 rounded-xl border border-cyan-300 bg-cyan-50 p-5 dark:border-cyan-700 dark:bg-cyan-950/30">
        <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold uppercase text-cyan-700 dark:text-cyan-400">
          <ExternalLink className="h-4 w-4" />
          Identificador operativo
        </h2>
        <p className="mb-3 text-xs text-zinc-600 dark:text-zinc-300">
          Usa <code className="rounded bg-cyan-100 px-1.5 py-0.5 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300">{tenant.slug}</code> como identificador operativo en metadata de llamadas y webhooks.
        </p>
        <pre className="rounded-lg bg-zinc-100 p-3 font-mono text-sm text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
          <div className="mb-4 flex items-center justify-between">
            ...
            {!editing ? (
              <button
                ...
                className="rounded-lg border border-zinc-300 p-1.5 text-zinc-500 transition hover:border-cyan-500 hover:text-cyan-600 dark:border-zinc-600 dark:text-zinc-400 dark:hover:border-cyan-500 dark:hover:text-cyan-400"
                title="Editar"
              >
              ...
              <button
                ...
                disabled={saving}
                className="rounded-lg border border-emerald-300 p-1.5 text-emerald-600 transition hover:bg-emerald-50 dark:border-emerald-700 dark:text-emerald-400 dark:hover:bg-emerald-950/30"
                title="Guardar"
              >
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">ID</label>
              <CopyableField value={tenant.id} label="Tenant ID" />
            </div>

            <div>
              <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Slug</label>
            ...
            {editing ? (
              <>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Nombre</label>
                  <input
                    type="text"
                    value={editName}
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Zona horaria</label>
                  <select
                    value={editTimezone}
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Estado</label>
                  <select
                    value={editStatus}
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs text-zinc-600 transition hover:bg-zinc-50 dark:border-zinc-600 dark:text-zinc-300 dark:hover:bg-zinc-800"
                  >
                    Cancelar
                  </button>
                </div>
              ...
              <>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-zinc-900 dark:text-zinc-100">{tenant.name}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-600 dark:text-zinc-400">Zona horaria</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-600 dark:text-zinc-400">Ready for calls</span>
                  {tenant.is_ready_for_calls ? (
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-zinc-500/10 px-2 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-500/20 dark:text-zinc-400">
                      <Clock className="h-3 w-3" />
                      No
                    </span>
                  )}
        ...
        <section className="space-y-4">
          <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <h3 className="mb-3 flex items-center gap-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">
              <Users className="h-4 w-4" />
              Membresías
            </h3>
            <p className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">{tenant.memberships.length}</p>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">usuarios conectados</p>
          </div>
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <p className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">{tenant.agents.length}</p>
      ...
      {/* Memberships Section */}
      <section className="mt-6 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-lg font-medium text-zinc-900 dark:text-zinc-100">
        ...
        {showAddMembership && (
          <div className="mb-4 rounded-lg border border-zinc-300 bg-zinc-50 p-4 dark:border-zinc-600 dark:bg-zinc-800">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Email del usuario *</label>
                <input
                  type="email"
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Rol</label>
                <select
              ...
              <div className="flex items-end gap-2">
                <button
                  ...
                  className="rounded-lg bg-cyan-600 px-4 py-2 text-xs font-medium text-white transition hover:bg-cyan-700 dark:bg-cyan-500 dark:hover:bg-cyan-400"
                >
                  Agregar
                </button>
                <button
                  ...
                  className="rounded-lg border border-zinc-300 px-4 py-2 text-xs text-zinc-600 transition hover:bg-zinc-100 dark:border-zinc-600 dark:text-zinc-300 dark:hover:bg-zinc-800"
                >
                  Cancelar
                </button>
              </div>
            ...
            {membershipError && (
              <p className="mt-2 text-xs text-red-600 dark:text-red-400">{membershipError}</p>
            )}
          </div>
        )}
        ...
        {tenant.memberships.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-200 text-left text-xs text-zinc-600 dark:border-zinc-700 dark:text-zinc-400">
      ...
      <section className="mt-6 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        {showAddAgent && (
          <div className="mb-4 rounded-lg border border-zinc-300 bg-zinc-50 p-4 dark:border-zinc-600 dark:bg-zinc-800">
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Nombre *</label>
                <input
                  type="text"
                  value={agentName}
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Provider *</label>
                <input
                  type="text"
                  value={agentProvider}
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Agent ID *</label>
                <input
                  type="text"
                  value={agentAgentId}
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-600 dark:text-zinc-400">Canal</label>
                <select
                  value={agentChannel}
            ...
            {agentError && (
              <p className="mt-2 text-xs text-red-600 dark:text-red-400">{agentError}</p>
            )}
          </div>
        )}
              ...
              <tbody>
                {tenant.agents.map((a) => (
                  <tr key={a.id} className="border-b border-zinc-100 dark:border-zinc-800">
                    <td className="py-2.5 pr-4 text-zinc-700 dark:text-zinc-200">{a.name}</td>
                    <td className="py-2.5 pr-4 text-zinc-600 dark:text-zinc-300">{a.external_provider}</td>
                    <td className="py-2.5 pr-4 font-mono text-zinc-700 dark:text-zinc-200">{a.external_agent_id}</td>
                    <td className="py-2.5 pr-4 text-zinc-600 dark:text-zinc-300">{a.channel_type || '—'}</td>
                    <td className="py-2.5">
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 dark:bg-black/70">
          <div
            ...
            className="w-full max-w-md rounded-xl border border-red-300 bg-white p-6 shadow-2xl dark:border-red-700 dark:bg-zinc-900"
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              ...
              <button
                ...
                disabled={deleting}
                className="rounded-lg p-1 text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-700 disabled:cursor-not-allowed disabled:opacity-50 dark:text-zinc-500 dark:hover:bg-zinc-800 dark:hover:text-zinc-200"
                title="Cerrar"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/30">
              ...
              <p className="mt-2 select-all rounded-md bg-zinc-50 px-3 py-2 font-mono text-lg font-semibold tracking-[0.2em] text-red-600 dark:bg-zinc-800 dark:text-red-400">
                {deleteCode}
              </p>
            </div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 font-mono text-sm uppercase tracking-wider text-zinc-900 placeholder:text-zinc-400 focus:border-red-400 focus:outline-none focus:ring-1 focus:ring-red-400 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:border-red-300 dark:focus:ring-red-300"
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                onClick={handleCloseDeleteModal}
                disabled={deleting}
                className="rounded-lg border border-zinc-300 px-4 py-2 text-sm text-zinc-600 transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-600 dark:text-zinc-300 dark:hover:bg-zinc-800"
              >
                Cancelar
              </button>
              <button
