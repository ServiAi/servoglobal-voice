...
import { ArrowRight, FileText, KeyRound, Send, Workflow } from 'lucide-react';
...
export default async function WhatsAppOverviewPage({ params }: Props) {
  ...
  const [configResult, templatesResult, flowsResult, t] = await Promise.all([
    fetchWhatsAppConfig(accessToken),
    fetchWhatsAppTemplates(accessToken),
    fetchWhatsAppFlows(accessToken),
  ...
  const actions = [
    ...
    { href: `${base}/flows`, icon: Workflow, label: t('quickActions.flows') },
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label={t('overview.summaryLabel')}>
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-xs"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{t('overview.flows')}</p><p className="mt-3 text-2xl font-bold tabular-nums text-foreground">{flowsResult.ok ? flowsResult.data.length : '—'}</p></div>
      </section>
