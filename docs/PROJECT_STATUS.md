...
| Voice Context Experiences | Builder privado operativo en código | Feature flag, context schemas versionados, administración tenant y UI bilingüe para inventario, wizard, edición, preview local (tres estados), estados y versiones. Cada campo del builder y de los esquemas incluye ayuda contextual con obligatoriedad. `published` es un snapshot interno presentado como "Versión preparada"; no hay URL pública, captura pública ni runtime WebRTC. Ver Etapa 0 en `docs-local/fase-4/VOICE_EXPERIENCE_FUNCTIONAL_ALIGNMENT.md`. |
