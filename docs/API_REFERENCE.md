...
- `POST /api/v1/agents/{agent_id}/delete` elimina definitivamente un agente archivado y sus versiones. Conserva sesiones/eventos e IDs históricos (`deleted_agent_id`, `deleted_agent_version_id`). Una sesión no terminal sin actualización durante 24 horas se cierra como `failed` (`agent_deleted_stale_session`) antes de desvincularse; una reciente responde `409` con `agent_delete_has_recent_session`. Un agente no archivado responde `409` con `agent_delete_requires_archived`. `DELETE /api/v1/agents/{agent_id}` se conserva por compatibilidad.
