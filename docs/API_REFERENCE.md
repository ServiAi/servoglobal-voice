...
| `/api/v1/tools/custom` | CRUD y ciclo de vida de Custom HTTP Tools tenant-scoped (`custom.*`), gateado por `custom_http_tools_v1`. |
...
- `GET /api/v1/agents/tools/catalog` (roles de lectura, no ligado a un `agent_id`) devuelve el catálogo unificado — `app.domain.tool_registry` (plataforma) más las Custom HTTP Tools activas del tenant (`custom.*`) — anotado con `available: bool` y `source: "platform"|"custom"` por entrada. `available=true` sólo si `status="available"` y, para plataforma, `required_integration` está configurada, o para custom, la credencial está configurada (o `auth_type="none"`) — misma resolución real que usa el preflight de publish en ambos casos. Las tools `status="planned"` siempre reportan `available=false`; una tool custom `disabled` no aparece en este catálogo (ver `GET /api/v1/tools/custom` para el listado completo con su estado real).
...
- `/api/v1/integrations/voice/providers/{provider}/agents` y `.../voices` exponen catálogos tenant-scoped con cursores opacos. `POST .../agents/{agent_id}/import` crea un agente/draft independiente. `GET .../voices/{voice_id}/preview` valida primero el recurso y obtiene la muestra del endpoint de preview; responde `audio/wav` o `audio/mpeg` según firma de bytes, con `Cache-Control: private, no-store`, sin confiar en MIME ni extensión upstream. Nunca se exponen `definition` ni `previewUrl`. `{provider}` se resuelve vía `voice_provider_admin.get_provider_admin_service`, que solo despacha a proveedores `status="active"` en `voice_registry.py` con un adapter real registrado; hoy únicamente `ultravox`. Cualquier otro valor responde `404 provider_not_available`.
- El preview del catálogo Ultravox admite WAV real aunque la cabecera MIME sea distinta, pero rechaza JSON/HTML y archivos mayores de 5 MB. Sigue hasta dos redirecciones HTTPS sólo hacia subdominios Ultravox o el host exacto `storage.googleapis.com` usado por muestras visibles en la consola Ultravox; envía `X-API-Key` únicamente al host exacto `api.ultravox.ai`. Una redirección fuera de ese ámbito responde `409 provider_preview_redirect_blocked`; los diagnósticos registran sólo estado HTTP, clase MIME y tamaño, sin URLs, claves ni contenido.
- `POST .../providers/ultravox/external-voice/preview` (roles de escritura: genera audio, potencialmente facturable) recibe el contrato ServiGlobal (`mode="provider_external"`, `provider="elevenlabs"`, `voice_id`, `settings`), lo valida localmente, construye `definition.elevenLabs` con un mapper explícito y llama `POST /api/voice_preview` de Ultravox con la API key Ultravox tenant-scoped (un único intento, sin retry). Responde `audio/wav` o `audio/mpeg` según la firma de bytes, sin confiar en el MIME upstream, con `Cache-Control: private, no-store`, sin persistir nada; ServiGlobal nunca resuelve ni almacena una credencial ElevenLabs propia.
...
Prefijo: `/api/v1/tools/custom`. Gateado por la feature tenant `custom_http_tools_v1` (mismo mecanismo que `agent_builder_v2`); toda operación deriva el tenant de `AuthContext`, nunca del body. Lectura: `platform_admin`, `tenant_admin`, `tenant_analyst`, `tenant_viewer`. Escritura (incluye `test`): `platform_admin`, `tenant_admin`.

| Método y ruta | Propósito |
| --- | --- |
| `GET /` | Lista las Custom HTTP Tools del tenant (cualquier estado). |
| `POST /` | Crea una tool en estado `disabled`. `key` debe matchear `custom.[a-z0-9_]{1,60}`; namespaces de plataforma (`calendar.*`/`whatsapp.*`/`crm.*`/`handoff.*`) se rechazan. `key` duplicada responde `409`. |
| `GET /{tool_id}` | Detalle completo (config + credencial enmascarada). |
| `PATCH /{tool_id}` | Actualiza cualquier campo salvo `key` (inmutable tras creación). `secrets` omitido/vacío conserva la credencial existente; con un `auth_type` distinto sin secreto nuevo, sólo se declara el tipo (`declare_auth_type`) sin borrar la credencial anterior salvo que de verdad cambie el mecanismo. |
| `DELETE /{tool_id}` | Elimina la tool. `409 tool_in_use` si alguna versión **publicada** de algún agente del tenant la referencia (un binding sólo en draft no bloquea). |
| `POST /{tool_id}/test` | Ejecuta la tool con argumentos de prueba reutilizando el mismo `CustomHttpToolExecutor` del runtime real (no un camino separado); no exige `status="active"`. Responde `{success, status_code, latency_ms, mapped_result, error_code}`, nunca cabeceras, credenciales o la URL cruda. |
| `POST /{tool_id}/activate` | Pasa a `status="active"`. Falla (`422`) si `auth_type != "none"` y no hay una credencial con secreto configurado. |
| `POST /{tool_id}/disable` | Pasa a `status="disabled"`; una tool deshabilitada nunca se resuelve como ejecutable en `ToolDispatchService`, incluso si un binding la sigue marcando `enabled`. |

Ningún endpoint de este prefijo devuelve un secreto en texto plano: `PATCH`/`POST` aceptan `secrets` como campo de solo escritura; toda respuesta expone `credential: {auth_type, configured, api_key_header_name, masked_fields}` con valores enmascarados (`SecretManager.mask_secret`, mismo patrón `abcd...wxyz`). `auth_type` soportados: `none`, `bearer` (`{token}`), `api_key` (`{api_key}` + `api_key_header_name`), `basic` (`{username, password}`).

`base_url` debe ser una URL HTTP(S) absoluta sin credenciales embebidas. Los headers sensibles (`Authorization`, cookies y nombres de API key/token/secret) se rechazan como configuración estática y deben declararse mediante `auth_type`/`secrets`. `input_schema` admite propiedades `string`, `object`, `array`, `integer`, `number`, `boolean` y `null`; el runtime valida esos tipos antes del request. Cualquier respuesta upstream `4xx`/`5xx`, timeout de conexión o lectura, redirect, MIME no permitido o cuerpo mayor de 1 MiB se reporta como fallo sanitizado.
...
- `GET /api/v1/crm/leads/{lead_id}/calls` añade `agent_name` opcional al historial, sin exponer secretos. `GET /api/v1/dashboard/recent-calls` entrega `channel` y `direction` opcionales además de `external_provider`; las proyecciones WebRTC sin CRM también aparecen allí y en la página global Llamadas. Las actividades de llamadas con contexto CRM exponen `transcript` como turnos `{event_id, sequence, occurred_at, speaker, text}` de usuario/asistente, más proveedor, canal y dirección.
