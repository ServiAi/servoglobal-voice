...
Ese usuario `administrator` es técnico, no un agente humano: su `id`/`email` quedan en `TenantChatwootConfig.managed_user_id`/`managed_user_email`, y `ChatwootConfigService.list_agents()` lo filtra explícitamente para que nunca aparezca en `GET /api/v1/integrations/chatwoot/agents` ni en la UI. En modo "external" estas columnas quedan `NULL` (no hay usuario técnico que excluir).

**Aislamiento entre tenants (`base_url` + `account_id`).** `tenant_chatwoot_configs` tiene `UNIQUE(base_url, account_id)` (migración `202609030001_chatwoot_account_isolation.py`) y `ChatwootConfigService.assert_account_available_for_tenant()` la valida en la capa de servicio antes de cualquier `INSERT`/`UPDATE`, devolviendo **409** (mensaje genérico, sin revelar el tenant dueño) si dos tenants intentan apuntar a la misma Account. El mismo `account_id` numérico en dos instancias Chatwoot distintas (`base_url` diferente) es válido y no colisiona. Si al desplegar esta migración en un entorno con datos existentes hay Accounts compartidas por más de un tenant, el `upgrade()` aborta con `RuntimeError` listando los duplicados en vez de aplicar la restricción a ciegas — resuélvalos a mano (reasignar o desactivar la configuración duplicada) antes de reintentar.
