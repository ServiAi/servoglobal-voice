...
**Custom HTTP Tools** extiende el mismo mecanismo de tools a herramientas HTTP declarativas por tenant, sin crear un segundo camino de function calling. `app.domain.tool_registry` sigue siendo el catálogo estático de tools de plataforma (con handlers Python en `_HANDLERS`); `tenant_tools`/`tenant_http_tool_configs`/`tenant_tool_credentials` (`backend/app/models/tools.py`) son el catálogo DB-backed de tools `custom.*` por tenant, reservando `calendar.*`/`whatsapp.*`/`crm.*`/`handoff.*` a plataforma (`app.domain.tool_namespace`). `ToolResolverService` (`backend/app/services/tool_resolver_service.py`) es el único punto que fusiona ambas fuentes en un `ResolvedToolDefinition` (`key/name/description/status/input_schema/source/required_integration/custom_tool_id`); `AgentService` (catálogo, validación de bindings al guardar draft, preflight de publish), `AgentCompilerService` (compilación) y `ToolDispatchService` (invocación) resuelven siempre a través de él en vez de reimplementar la rama plataforma/custom cada uno. `CompiledToolSpec` (`key/name/description/input_schema`) es idéntico para ambas fuentes — una tool custom nunca lleva `base_url`/`credencial` al runtime, así que `voice-runtime/tool_dispatcher.py` no requiere ningún cambio: sigue registrando cada tool como `RawFunctionTool` genérico. `ToolDispatchService.invoke()` bifurca al inicio por prefijo `custom.` hacia `CustomHttpToolExecutor` (`backend/app/services/custom_http_tool_executor.py`) sin tocar el camino de las tools de plataforma; el executor valida argumentos con el mismo validador que `_HANDLERS`, resuelve la credencial vía `TenantToolCredentialService` (Fernet + `INTEGRATIONS_ENCRYPTION_KEY`, igual que `TenantIntegration`; nunca decodifica salvo para el propio executor), construye path/query/body con mappings declarativos de solo lectura de claves (`app.domain.tool_mapping`, sin `eval`) sobre raíces permitidas (`args`/`context`/`binding`), y llama `SafeHttpClient` (`backend/app/services/tool_http_safety.py`): scheme `https` únicamente en producción, resuelve DNS y valida cada IP contra rangos privados/loopback/link-local/metadata antes de conectar, pinea la conexión a la IP ya validada preservando SNI/verificación TLS contra el hostname original (evita DNS rebinding), sin redirects automáticos, con timeout y tamaño de respuesta acotados. `TenantFeatureService` gatea todo el dominio tras `custom_http_tools_v1`, mismo patrón que `agent_builder_v2`. `POST /api/v1/tools/custom/*` (`backend/app/api/endpoints/tools_custom.py`) es el CRUD tenant-scoped; el endpoint interno de invocación (`POST .../tools/{key}/invoke`) no cambia de contrato. Ver `docs/CUSTOM_TOOLS.md` para el detalle completo de seguridad, notación de mapping y flujo de ejecución.
...
`VoiceCallProjectionService` consume `VoiceSession` y sus eventos para todos los canales reales del runtime. Una llamada CRM operativa o evidencia de participante/conversación activa un `analytics.Call` único con `external_call_id=voice-session:{session_id}`; `external_provider` es el proveedor de IA, mientras `channel`, `direction` y `runtime_engine` permanecen conceptos separados. `CrmVoiceCall` sólo se actualiza si ya está enlazada; `CrmActivity` sólo se crea para contacto/lead validado del mismo tenant, por lo que una prueba WebRTC sin CRM aparece en Historial global pero nunca en Conversaciones de un lead. El runtime guarda mensajes comprometidos de usuario y asistente como `voice.transcript.final` en `VoiceSessionEvent`; la actividad CRM expone una proyección ordenada de esos eventos cuando hay contexto. El backfill manual ejecuta el mismo servicio sin inferir cierres ausentes.
