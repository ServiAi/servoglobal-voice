...
**El límite de aislamiento multi-tenant de Chatwoot es la Account, nunca el Inbox.** `tenant_chatwoot_configs` tiene `UNIQUE(base_url, account_id)` (mismo par sólo puede pertenecer a un tenant; `account_id` solo no alcanza porque dos instancias Chatwoot distintas pueden reusar el mismo id numérico) reforzado además en `ChatwootConfigService.assert_account_available_for_tenant()` antes de cualquier persistencia (409 si la Account ya es de otro tenant, sin filtrar su identidad). Todo endpoint de tenant (`inboxes`, `teams`, `agents`) deriva `base_url`/`account_id`/token exclusivamente de `TenantChatwootConfig` vía `AuthContext.tenant_id` — nunca de la URL, el body ni un `account_id` enviado por el frontend — por lo que un `inbox_id`/`team_id`/`agent_id` adivinado de otro tenant nunca puede alcanzar la Account de ese otro tenant. El modo "managed" crea además un usuario técnico `administrator` (`chatwoot-managed+{account_id}@serviglobal-ia.com`, columnas `managed_user_id`/`managed_user_email`) para operar la Account por API; `ChatwootConfigService.list_agents()` lo excluye explícitamente para que sólo aparezcan agentes humanos.
