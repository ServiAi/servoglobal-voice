...
class CrmVoiceCall(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_crm_voice_calls_tenant_lead", "tenant_id", "lead_id"),
...
class CrmVoiceCallEvent(Base):
    __tablename__ = "crm_voice_call_events"
    __table_args__ = (
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    tenant_id: Mapped[str] = mapped_column(ForeignKey("tenants.id"), nullable=False)
    voice_call_id: Mapped[str] = mapped_column(ForeignKey("crm_voice_calls.id"), nullable=False)
    ...
    tenant = relationship("Tenant")
    voice_call = relationship("CrmVoiceCall")
